...
import com.nbteam.hislite.common.utils.HttpCallback;
...
public class HttpClientHelper implements HttpHelper {
    ...
    int maxTotalConnections = 4096;

    int defaultMaxConnectionsPerHost = 1024;
     ...
     *            the defaultMaxConnectionsPerHost to set
     */
    public void setDefaultMaxConnectionsPerHost(int defaultMaxConnectionsPerHost) {
        this.defaultMaxConnectionsPerHost = defaultMaxConnectionsPerHost;
        this.manager.getParams().setDefaultMaxConnectionsPerHost(
    ...
    public HttpClientHelper(boolean initProxy) {
        ...
        if (initProxy) {
            try {
                ...
                String proxyHost = HttpClientHelper.trimToNull(System
                        .getenv(HttpClientHelper.ENV_PROXY_KEY));
                proxyHost = System.getProperty(HttpClientHelper.ENV_PROXY_KEY,
                        proxyHost);
                ...
                if (proxyHost != null) {
                    ...
                    HttpClientHelper.logger
                            .warn("HttpClientHelper no proxy to set");
                }
            } catch (Exception e) {
    ...
    protected HttpState setHttpState(HttpState httpState,
            Map<String, String> cookies, String url) throws IOException {
        if (httpState == null) {
            httpState = new HttpState();
        }
        if (cookies != null && cookies.size() != 0) {
            ...
            for (Entry<String, String> en : cookies.entrySet()) {
                ...
                if (n != null) {
                    httpState.addCookie(new Cookie(host, n, en.getValue(), "/",
                            null, false));
                }
            }
        }
    ...
    protected HttpState setHttpState(HttpState httpState, Cookie[] cookies,
    ...
    protected <T> HttpMethod getGetMethod(String uri, Map<String, T> data,
            String charset, Header[] headers) {
        return this.setParameter(this.getGetMethod(uri, charset, headers),
                data, charset);
    }

    protected HttpMethod getGetMethod(String uri, String queryStr,
            String charset, Header[] headers) {
        return this.setParameter(this.getGetMethod(uri, charset, headers),
                queryStr);
    }
    ...
    protected <T> HttpMethod getGetMethod(String uri, Map<String, T> data,
        ...
        return this.getGetMethod(uri, data, charset,
                this.createHeaders(headers));
    }

    protected GetMethod getGetMethod(String uri, String charset,
            Header[] headers) {
    ...
    protected <T> GetMethod setParameter(GetMethod getMethod,
            Map<String, T> data, String charset) {
        if (charset == null) {
            charset = HttpHelper.DEFAULT_CHARSET;
        }
            ...
            for (Entry<String, T> entry : data.entrySet()) {
                ...
                if (value != null) {
                    if (value instanceof String) {
                        paras.add(new NameValuePair(entry.getKey(),
                                (String) value));
                        ...
                        paras.add(new NameValuePair(entry.getKey(), String
                                .valueOf(value)));
                    }
                }
            }
            if (CollectionUtils.isNotEmpty(paras)) {
                String qStr = HttpClientHelper.trimToNull(getMethod
                        .getQueryString());
                if (qStr != null) {
                    getMethod.setQueryString(qStr
                            + "&"
                            + EncodingUtil.formUrlEncode(paras
                                    .toArray(new NameValuePair[paras.size()]),
                                    charset));
                } else {
                    getMethod.setQueryString(EncodingUtil.formUrlEncode(
                            paras.toArray(new NameValuePair[paras.size()]),
                            charset));
                }
            }
        }
    ...
    protected GetMethod setParameter(GetMethod getMethod, String queryStr) {
    ...
    protected Header[] createHeaders(Map<String, String> headers) {
        if (headers != null && headers.size() != 0) {
            List<Header> hs = new ArrayList<>(headers.size());
    ...
    protected HttpMethod getPostMethod(String uri, String charset,
            Map<String, String> headers, InputStream data) {
        return this.getPostMethod(uri, charset, this.createHeaders(headers),
                data);
    }

    protected HttpMethod getPostMethod(String uri, String charset,
            Header[] headers, InputStream data) {
        return this.setParameter(this.getPostMethod(uri, charset, headers,
                "application/octet-stream"), data);
    }

    protected <T> HttpMethod getPostMethod(String uri, String charset,
            Header[] headers, Map<String, T> data) {
        return this.setParameter(this.getPostMethod(uri, charset, headers,
                HttpClientHelper.DEFAULT_POST_CONTENT_TYPE), data);
    }

    protected <T> HttpMethod getPostMethod(String uri, String charset,
            Map<String, String> headers, Map<String, T> data) {
        return this.getPostMethod(uri, charset, this.createHeaders(headers),
                data);
    }

    protected PostMethod getPostMethod(String uri, String charset,
            Header[] headers, String contentType) {
        ...
        if (headers != null && headers.length != 0) {
            for (Header h : headers) {
                ct = ct
                        || HttpHelper.HEADER_NAME_CONTENT_TYPE
                                .equalsIgnoreCase(h.getName());
                postMethod.addRequestHeader(h);
            }
        }
        if (!ct) {
            postMethod.addRequestHeader(HttpHelper.HEADER_NAME_CONTENT_TYPE,
                    contentType);
        }
        return postMethod;
    }
    ...
    protected <T> PostMethod setParameter(PostMethod postMethod,
            Map<String, T> data) {
        if (data != null && data.size() > 0) {
            for (Entry<String, T> entry : data.entrySet()) {
                Object value = entry.getValue();
     ...
     * (non-Javadoc)
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#get(java.lang.String,
     * java.util.Map)
     */
    @Override
     ...
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#get(java.lang.String,
     * java.util.Map, com.iqiyi.ugc.utils.commonhttp.HttpCallback)
     */
    @Override
    public <T> int get(String address, Map<String, T> data,
            HttpCallback callback) throws IOException {
        return this.get(address, data, HttpHelper.DEFAULT_CHARSET,
                (Cookie[]) null, (Header[]) null, callback);
    }
     ...
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#get(java.lang.String,
     * java.util.Map, java.lang.String, org.apache.commons.httpclient.Cookie[],
     * org.apache.commons.httpclient.Header[],
     * com.iqiyi.ugc.utils.commonhttp.HttpCallback)
     */
    @Override
    public <T> int get(String address, Map<String, T> data, String charset,
            Cookie[] cookies, Header[] headers, HttpCallback callback)
            throws IOException {
        return this.exec(this.getGetMethod(address, data, charset, headers),
     ...
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#get(java.lang.String,
     * java.util.Map, java.lang.String, java.util.Map, java.util.Map,
     * com.iqiyi.ugc.utils.commonhttp.HttpCallback)
     */
    @Override
    public <T> int get(String address, Map<String, T> data, String charset,
    ...
    public int get(String address, String queryStr, String charset,
            ...
            throws IOException {
        return this.exec(
                this.getGetMethod(address, queryStr, charset, headers),
                this.setHttpState(null, cookies, address), callback);
    }
     ...
     * (non-Javadoc)
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#post(java.lang.String,
     * java.util.Map)
     */
    @Override
    ...
    public <T> int post(String address, Map<String, T> data,
            HttpCallback callback) throws IOException {
        return this.post(address, data, HttpHelper.DEFAULT_CHARSET,
                (Cookie[]) null, (Header[]) null, callback);
    }
    ...
    public <T> int post(String address, Map<String, T> data, String charset,
            Map<String, String> cookies, Map<String, String> headers,
            HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, data),
                this.setHttpState(null, cookies, address), callback);
    }
     ...
     * 
     * @see com.iqiyi.ugc.utils.commonhttp.HttpHelper#post(java.lang.String,
     * java.io.InputStream, java.lang.String, java.util.Map, java.util.Map,
     * com.iqiyi.ugc.utils.commonhttp.HttpCallback)
     */
    @Override
    public int post(String address, InputStream data, String charset,
    ...
    public int post(String address, byte[] data, String charset,
            Map<String, String> cookies, Header[] headers, HttpCallback callback)
            throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers,
                new ByteArrayInputStream(data)), this.setHttpState(null,
                cookies, address), callback);
    }
    ...
    public int post(String address, String data, String charset,
        ...
        if (StringUtils.isNotBlank(contentType)) {
            headers[0] = new Header(HttpHelper.HEADER_NAME_CONTENT_TYPE,
                    contentType);
        } else {
            headers[0] = new Header(HttpHelper.HEADER_NAME_CONTENT_TYPE,
                    "text/plain");
        }
        return post(address, getBytes(data, charset), charset, null, headers,
                callback);
    }

    protected byte[] getBytes(String data, String charset)
	...
	public <T> int put(String address, Map<String, T> data,
			HttpCallback callback) throws IOException {
		return this.put(address, data, HttpHelper.DEFAULT_CHARSET,
                (Cookie[]) null, (Header[]) null, callback);
	}
	...
	public <T> int put(String address, Map<String, T> data, String charset,
			Cookie[] cookies, Header[] headers, HttpCallback callback)
			throws IOException {
		return this.exec(this.getPutMethod(address, charset, headers, data),
                this.setHttpState(null, cookies, address), callback);
	}
	...
	protected HttpMethod getPutMethod(String uri, String charset,
            Header[] headers, InputStream data) {
        return this.setEntity(this.getPutMethod(uri, charset, headers,
                "application/octet-stream"), data);
    }

    protected <T> HttpMethod getPutMethod(String uri, String charset,
            Header[] headers, Map<String, T> data) throws UnsupportedEncodingException {
        return this.setEntity(this.getPutMethod(uri, charset, headers,
                HttpClientHelper.DEFAULT_POST_CONTENT_TYPE), data, charset);
    }
    
    protected HttpMethod getPutMethod(String uri, String charset,
            Header[] headers, byte[] data) throws UnsupportedEncodingException {
        return this.setEntity(this.getPutMethod(uri, charset, headers,
                HttpClientHelper.DEFAULT_POST_CONTENT_TYPE), data);
    }
    
    protected HttpMethod getPutMethod(String uri, String charset,
            Header[] headers, String contentType, String data) throws UnsupportedEncodingException {
        return this.setEntity(this.getPutMethod(uri, charset, headers,
        		contentType), data, contentType, charset);
    }
    
    protected PutMethod getPutMethod(String uri, String charset,
            Header[] headers, String contentType) {
        ...
        if (!ct) {
        	putMethod.addRequestHeader(HttpHelper.HEADER_NAME_CONTENT_TYPE,
                    contentType);
        }
        return putMethod;
    }
    ...
    protected PutMethod setEntity(PutMethod putMethod, 
    		String data, String contentType, String charset) 
    				throws UnsupportedEncodingException {
        if (data != null) {
    ...
    protected <T> PutMethod setEntity(PutMethod putMethod, 
    		Map<String, T> data, String charset) 
    				throws UnsupportedEncodingException {
        if (data != null && data.size() > 0) {
            ...
            for (Entry<String, T> entry : data.entrySet()) {
                ...
                if (value != null) {
                    if (value instanceof String) {
                        ...
                        builder.append(URLEncoder.encode(temp.toString(), charset))
                        .append("&");
                    } else if (value.getClass().isArray()) {
                    	builder.append(URLEncoder.encode(entry.getKey(), charset)).append("=");
                    	StringBuilder temp = new StringBuilder();
